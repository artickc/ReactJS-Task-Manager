...
export const TaskCard: React.FC<TaskProps> = React.memo(function TaskCard({
 ...
 const d1 = Date.parse(task.deadline);
 const d2 = Date.parse(new Date().toString());
let deadlineColor = 'red';
var Difference_In_Time = new Date(task.deadline).getTime() - new Date().getTime();
var Difference_In_Days = Difference_In_Time / (1000 * 3600 * 24);
if(Difference_In_Time < 0)deadlineColor = 'red';
else if(Difference_In_Time > 0 && Difference_In_Days > 0)deadlineColor = 'red';
else 
 ...
 return (
  ...
  >
   <Box sx={{minWidth: 275}}>
    <Card variant="outlined" sx{{backgroundColor: 'red'}}>
     <React.Fragment>
      <CardActions>
