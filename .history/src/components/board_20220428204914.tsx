...
export const Board: React.FC = () => {
 ...
 const handleTitleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  updateActive();
  if (workingObject.title.length < TITLE_MIN_LENGTH) setSaveState(true);
 };

 const updateActive = (clear: boolean = false) => {
  if (clear) setSaveState(true);
  else if (workingObject.title.length > TITLE_MIN_LENGTH) setSaveState(false);
  try {
   store.dispatch({
    ...
    >
     ...
     <DialogContent>
      ...
      <TextField
       ...
       error={saveState && workingObject.title.length > 0 && !workingObject.id}
