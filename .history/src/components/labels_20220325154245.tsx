...
export const LabelsComponent = () => {
 ...
 const toggleLabel = (id: number) => (event: React.MouseEvent) => {
  ...
  let neetTasksUpdate: boolean = false;
  tasks.map((task: Tasks) => {
   if (active.some((v) => v === task.id)) {
    var index = tasks.indexOf(task.id);
    if (index !== -1) tasks.splice(index, 1);
    if (!neetTasksUpdate) neetTasksUpdate = true;
   }
  });
  if (neetTasksUpdate === true) {
   try {
    store.dispatch({
