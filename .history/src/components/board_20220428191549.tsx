...
export const Board: React.FC = () => {
 ...
 let titleValidation: boolean = true;
 ...
 React.useEffect(() => {
  ...
  if (text.length < TITLE_MIN_LENGTH) {
   setErrorMessage("The input has exceeded the minimum number of characters");
  }
 }, [text]);
 ...
 const handleTitleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  updateActive();
  if (workingObject.title.length > TITLE_MIN_LENGTH) titleValidation = false;
 };
 ...
 const handleSave = () => {
  if (!titleValidation) return false;
  try {
   store.dispatch({
    type: AppActionTypes.OPEN_APP_DIALOG,
    payload: false,
    ...
    >
     ...
     <DialogContent>
      ...
      <TextField
       ...
       error={workingObject.title.length <= TITLE_MIN_LENGTH}
