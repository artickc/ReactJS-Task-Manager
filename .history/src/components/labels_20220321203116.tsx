      ...
      >
                ...
                labels.map(label=>
                    <div style={{
                        ...
                        border: '1px solid',
                        color: label.color,
                        borderColor: useTypeSelector(state => state.labels.active).some(v => (v === label.id))?label.color:"transparent"    
                    }} key={label.id} onClick={() => toggleLabel(label.id)} className={useTypeSelector(state => state.labels.active).some(v => (v === label.id))?'labelItem active': 'labelItem'}>
                        {label.title}
                        {useTypeSelector(state => state.labels.active).some(v => (v === label.id))?<CloseIcon color="error" />:<AddIcon color="success" />}
                    </div>
        )}
