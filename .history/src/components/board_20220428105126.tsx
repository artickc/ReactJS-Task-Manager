...
export const Board: React.FC = () => {
  ...
  axios.post("/api/tasks/", {task: workingObject}).then((res) => {
   if (res.data.result) {
    ...
    try {
     ...
     store.dispatch({
      type: AppActionTypes.FETCH_APP_ERROR,
      payload: "Error while creating tasks in task list",
     });
    }
   } else showError("Error while creating tasks in task list");
  });
 };
 const deleteTask = () => {
  ...
  axios.post("/api/tasks/" + id + "/delete").then((res) => {
   if (res.data.result) {
    try {
     ...
     store.dispatch({
      type: AppActionTypes.FETCH_APP_ERROR,
      payload: "Error while deleting tasks in task list",
     });
    }
   } else showError("Error while deleting tasks in task list");
  });
 };
 const updateTask = () => {
  axios
   ...
   .then((res) => {
    if (res.data.result) {
     ...
     try {
      ...
      store.dispatch({
       type: AppActionTypes.FETCH_APP_ERROR,
       payload: "Error while updating tasks in task list",
      });
     }
    } else {
     showError("Error while updating tasks in task list");
    }
   });
 };
 ...
 const handleDrop = (card: Tasks, item: ColumnState) => {
  card.status = item.id;
  axios.post("/api/tasks/" + card.id + "/status", {status: card.status});
  try {
 ...
 return (
  <div className="boardContainer">
   {alertV ? (
    <Alert
     sx={{position: "fixed", bottom: "8px", right: "12px"}}
     severity="error"
    >
     {alertContent}
    </Alert>
   ...
   <div>
    ...
    >
     ...
     <DialogActions>
      ...
      <Button sx={{color: "success.dark"}} onClick={handleSave}>
       Save
      </Button>
     </DialogActions>
    </Dialog>
