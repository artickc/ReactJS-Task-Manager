...
export const LabelsComponent: React.FC = () => {
    ...
    return(
      ...
      >
        ...
        return  <Item key={label.id} onClick={toggleLabel(label.id)} className={"labelItem "+useTypeSelector(state => state.labels.active).some(v => (v === label.id))?'active': '')} > {label.title} <AddIcon /> </Item>
    }
        )}
