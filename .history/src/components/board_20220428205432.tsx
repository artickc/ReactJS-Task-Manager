...
export const Board: React.FC = () => {
 ...
 const handleChange = (newValue: Date | null, active: Tasks) => {
  ...
  if (workingObject.title.length > TITLE_MIN_LENGTH) setSaveState(false);
 };
 ...
 const handleTitleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  updateActive();
  if (workingObject.title.length < TITLE_MIN_LENGTH) {
   setSaveState(true);
   setTitleError(true);
 ...
 const updateActive = (clear: boolean = false) => {
  if (clear) {
   setSaveState(true);
   setTitleError(false);
  } else if (workingObject.title.length > TITLE_MIN_LENGTH) {
   setSaveState(false);
   setTitleError(false);
  }
  try {
