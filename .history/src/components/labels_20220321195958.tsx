...
export const LabelsComponent: React.FC = () => {
    ...
    let active = useSelector(state: state.labels.active) 
    ...
    return(
      ...
      >
                ...
                labels.map(label=>
        {
                ...
                let isOn = active.some(v => (v === label.id))?'active': false;
                 let Item = styled(Paper)(({ theme }) => (
                    {
                        ...
                        border: '1px solid',
                        color: label.color,
                        borderColor: isOn?label.color:"transparent"
                    }));


        return  <Item key={label.id} onClick={toggleLabel(label.id)} className={"labelItem "+isOn} > {label.title} <AddIcon /> </Item>
    }
        )}
