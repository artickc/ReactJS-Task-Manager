...
export const TaskCard: React.FC<TaskProps> = React.memo(function TaskCard({
 ...
 const [deadLinedate, deadLinetime] = task.deadline.split(" ");
 const [dyear, dmonth, dday] = deadLinedate.split("-");
 const [dhours, dminutes, dseconds] = deadLinetime.split(":");
 const deadline = `${dyear}-${dmonth}-${dday} ${dhours}:${dminutes}:${dseconds}`;

 const d2 = Date.parse(deadline);

 console.log(d1, d2, deadline);

 var Difference_In_Time = d1 - new Date(d2).getTime();
