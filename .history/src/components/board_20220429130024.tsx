...
export const Board: React.FC = () => {
 ...
 const textEditorRef: any = React.useRef();
 ...
 const handleDateChange = (newValue: Date | null) => {
  ...
  // workingObject.object.deadline = format(newValue, "dd-MM-Y HH:mm:ss");
  // setEdited(true);
 };
 ...
 const onEditorChange = (data: any) => {
  ...
  let rteContent = stateToHTML(content);
  rteContent = rteContent.replace("<p><br></p>", "");
  if (rteContent != workingObject.object.description) {
   workingObject.object.description = rteContent;
   if (rteContent.length > 0) setEdited(true);
  }
 };
 ...
 const updateOrCreateTask = () => {
  if (workingObject.errors.length > 0) return false;
 ...
 const handleDelete = () => {
  confirm({
   ...
   dialogProps: {classes: {root: "confirmActionContainer"}},
  }).then(() => {
   try {
    store.dispatch({
     type: AppActionTypes.OPEN_APP_DIALOG,
    ...
    >
     ...
     <DialogContent>
      ...
      <TextField
       ...
       error={workingObject.errors.length ? true : false}
       defaultValue={workingObject.object.title}
       type="text"
       fullWidth
     ...
     <DialogActions>
      <Button className="cancelCardBtn" onClick={handleClose}>
       Cancel
      </Button>
      <Button
      ...
      <Button
       ...
       disabled={
        workingObject.errors.length ||
        !workingObject.edited ||
        workingObject.object.title.length < TITLE_MIN_LENGTH
         ? true
         : false
       }
