...
export const TaskCard: React.FC<TaskProps> = React.memo(function TaskCard({
 ...
 const handleClickOpen = (task: Tasks) => {
  let worckingObject = {
   object: Object.assign({}, task),
   errors: [],
   edited: false,
  };

  const contentHTML = convertFromHTML(worckingObject.object.description);
  const content = ContentState.createFromBlockArray(
   contentHTML.contentBlocks,
   contentHTML.entityMap
  );
  description = JSON.stringify(convertToRaw(content));
  worckingObject.object.description = description;

  try {
   store.dispatch({
    type: AppActionTypes.CHANGE_ACTIVE,
    payload: {...worckingObject},
   });
   store.dispatch({
 ...
 const carddeadline = `${dyear}-${dmonth}-${dday} ${dhours}:${dminutes}:${dseconds}`;

 const d1 = new Date().getTime();
 const d2 = Date.parse(carddeadline);
